...
LFU 알고리즘은 참조 횟수가 가장 적은 페이지를 교체하는 방법이다. 활발하게 사용되는 페이지는 그만큼 참조 횟수가 많을 것이라는 가정을 기본 아이디어로 한다. LRU 알고리즘이 페이지 참조의 최신성을 기준으로 한다면, LFU 알고리즘은 페이지의 참조 빈도 즉 인기도를 기준으로 한다.
