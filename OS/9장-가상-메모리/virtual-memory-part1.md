...
이 요구조건은 프로그램의 크기를 물리 메모리의 크기로 제한한다는 점 때문에 불편한 점이 많았다. 실제 프로그램들을 살펴보면, 사실 많은 경우에 프로그램 전체가 메모리에 항상 올라와 있을 필요는 없다는 것을 알 수 있다. 요구조건 때문에 불필요하게 메모리에 올라와 있는 것들로는 다음의 것들이 있다.
...
쓰레싱을 방지하기 위해서는 각 프로세스가 필요로 하는 갯수만큼 프레임을 할당해야 한다. 각 프로세스가 필요로 하는 프레임의 수를 알 수 있는 방법에는 다음의 두 가지가 있다.
