...
그래프 *G* = (*V*, *E*)에서 각 간선에 가중치 *w*를 둔다. 이 **가중치의 합을 최소로 하고 모든 정점을 연결하는 트리**를 최소 신장 트리라고 한다. 이 트리를 결정하는 문제를 해결하는 알고리즘으로 **크루스칼 알고리즘**, **프림 알고리즘**이 존재한다.
...
## 일반 해결법
그리디 알고리즘을 통해 최소 신장 트리 문제를 해결하는 방법은 모든 정점을 이을 때까지 한 번에 하나의 간선을 추가하는 방식으로 이루어지고 간선의 집합 *A*를 유지한다. 이 때 **가장 중요한 것은 *A*가 최소 신장 트리의 부분 집합**이라는 것이다.
